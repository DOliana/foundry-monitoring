...
Prerequisites:
- see [`/docs/RBAC_REQUIREMENTS.md`](docs/RBAC_REQUIREMENTS.md) for permissions
...
| File | Description |
|------|-------------|
| [`monitor-foundry.ipynb`](proof-of-concept/monitor-foundry.ipynb) | Collects quota, deployment, and token usage data across all subscriptions via Azure Monitor APIs. Visualizes subscription-level quotas, deployment rate limits, and hourly token usage with Plotly charts. |
| [`monitor-foundry-example.ipynb`](proof-of-concept/monitor-foundry-example.ipynb) | Queries a Log Analytics workspace for AzureMetrics data from Foundry instances. Demonstrates how to retrieve metrics and diagnostics via the Log Analytics SDK. |
...
| Module | Description |
|--------|-------------|
| [`main.bicep`](infrastructure/main.bicep) | Orchestrator — wires all modules together, references the existing Log Analytics workspace. |
...
| [`alerts.bicep`](infrastructure/modules/alerts.bicep) | Action group + alert rule for function failures. |

### PowerShell Scripts (`/scripts` folder)

| File | Description |
|------|-------------|
| [`Set-FoundryDiagnosticSettings.ps1`](scripts/Set-FoundryDiagnosticSettings.ps1) | Discovers all Foundry resources and creates diagnostic settings to stream metrics to a Log Analytics workspace. Supports `-Remove` and `-WhatIf`. |
| [`Invoke-FoundryTrafficGenerator.ps1`](scripts/Invoke-FoundryTrafficGenerator.ps1) | Sends test requests to model deployments (chat completions, embeddings) across Foundry instances. Supports parallel execution, multiple iterations, and `-WhatIf`. |
| [`Invoke-FoundryLoadTest.ps1`](scripts/Invoke-FoundryLoadTest.ps1) | Interactive load-test tool to hammer a deployed model (aka produce 429 😎) — drills down into subscription → instance → deployment, then fires parallel requests. Supports chat completions and embeddings. |
