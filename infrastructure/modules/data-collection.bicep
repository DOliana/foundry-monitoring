...
resource dcrModelCatalog 'Microsoft.Insights/dataCollectionRules@2023-03-11' = {
  name: '${prefix}-dcr-model-catalog'
  location: location
  tags: tags
  properties: {
    dataCollectionEndpointId: dce.id
    streamDeclarations: {
      'Custom-ModelCatalog_CL': {
        columns: [
          { name: 'TimeGenerated', type: 'datetime' }
          { name: 'subscriptionId_s', type: 'string' }
          { name: 'subscriptionName_s', type: 'string' }
    ...
    dataFlows: [
      {
        streams: ['Custom-ModelCatalog_CL']
        destinations: ['logAnalytics']
        transformKql: 'source'
        outputStream: 'Custom-ModelCatalog_CL'
      }
    ]
  }
}
...
output dcrModelCatalogId string = dcrModelCatalog.id
output dcrModelCatalogImmutableId string = dcrModelCatalog.properties.immutableId
